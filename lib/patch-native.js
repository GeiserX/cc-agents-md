...
function patchNative(binaryPath, options = {}) {
  ...
  // Native binary patching only handles a single reader function occurrence.
  // Multiple matches would result in partial patching — refuse rather than corrupt.
  if (matchCount !== 1) {
    return {
      success: false,
      message: `Expected exactly 1 reader function in binary, found ${matchCount}. Refusing to patch partially.\n` +
        'Use npm-installed Claude Code for multi-match patching:\n' +
