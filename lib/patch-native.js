 ...
 * inside the binary in the __BUN Mach-O section. This module finds all async
 * reader functions and patches each one individually with AGENTS.md fallback.
 *
 * Strategy:
 *   1. In-place byte replacement — if null padding exists after each function
 *   2. Partial patching — patch functions that fit, skip those that don't
 *   3. If none fit, fail with instructions to use npm
 *
 ...
 * Same structure as ASYNC_READER_RE but anchored, non-global.
 */
 ...
 * individually with AGENTS.md fallback. Functions without enough null
 * padding after them are skipped.
 *
...
function patchNative(binaryPath, options = {}) {
  ...
  const newBinary = Buffer.from(binary);
  ...
  if (patched === 0) {
    ...
    return {
      ...
      message: `Found ${matches.length} reader function(s) but none had room for the patch.\n` +
        'Native binary is tightly packed — no null padding between functions.\n' +
