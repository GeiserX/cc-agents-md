...
function buildReplacement(match, fnName, argPath, argType, argParam, varContent, fsAccessor, processor, catchVar, errorHandler) {
  // Compact replacement: single regex swaps CLAUDE→AGENTS (handles .local.md too)
  // /CLAUDE(\.local)?\.md$/ matches CLAUDE.md, CLAUDE.local.md, .claude/CLAUDE.md
  // $1 captures ".local" when present, so "AGENTS$1.md" produces the right filename
  return `${PATCH_SENTINEL}async function ${fnName}(${argPath},${argType},${argParam},_r){try{let ${varContent}=await ${fsAccessor}().readFile(${argPath},{encoding:"utf-8"});return ${processor}(${varContent},${argPath},${argType},${argParam})}catch(${catchVar}){if(!_r){let a=${argPath}.replace(/CLAUDE(\\.local)?\\.md$/,"AGENTS$1.md");if(a!==${argPath})try{return await ${fnName}(a,${argType},${argParam},1)}catch(e){}}return ${errorHandler}(${catchVar},${argPath}),{info:null,includePaths:[]}}}`;
}
